...
class QuestionRestorer:
        ...
        for area, cfg in self.bot.quiz_data.items():
            ...
            try:
                ...
                if end_time > datetime.datetime.utcnow():
                    ...
                    logger.info(
                        f"[Restorer] Abgelaufene Frage in '{area}' wird entfernt.")
    ...
    async def repost_question(self, area: str, qinfo: dict):
        ...
        if msg.embeds and (
            ...
            or "hat gewonnen" in msg.embeds[0].footer.text
        ):
            logger.warning(
        ...
        self.bot.loop.create_task(self.bot.quiz_cog._auto_close(area, delay))
